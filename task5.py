...
def random_coords(start,end):
    ...
    for i in range(200):
        ...
        if r==0:
            p_old=points[-1]
            p_new=[p_old[0]+np.random.randint(1,6),p_old[1]]
            points.append(p_new)
        elif r==1:
            p_old=points[-1]
            p_new=[p_old[0],p_old[1]+np.random.randint(1,6)]
            points.append(p_new)
    
    return points

def heatmap_image(height,width,p,k,gauss):
    img1=np.zeros((height,width,3)).astype(np.float32)
    for p in points:
        
        xi=int((p[0]-np.round(k/2)))
        xj=int((p[0]+np.round(k/2)))+1
        yi=int((p[1]-np.round(k/2)))
        yj=int((p[1]+np.round(k/2)))+1
        
        b=img1[xi:xj,yi:yj,:]
        c=gauss+b
        img1[xi:xj,yi:yj,:]=c
    print("gauss max",np.max(gauss),np.max(img1,axis=(0,1)))
    img1=img1/(np.max(img1,axis=(0,1)))+0.0001
    return img1
...
# mage=heat_map(im,200,200,21,H,W,gauss_kernel)
...
def merge_views(v1,v2,v3):
    v1 = cv2.resize(v1,(500,500))
    v2 = cv2.resize(v2,(500,500))
    v3 = cv2.resize(v3,(500,500))
...
#5. Computing hmography top-view and showing them
# view1 = cv2.warpPerspective(view1, H1, (w1, h1))
# cv2.namedWindow('view1',cv2.WINDOW_AUTOSIZE)
# cv2.imshow("view1",view1)
# view2 = cv2.warpPerspective(view2, H2, (w2, h2))
# cv2.namedWindow('view2',cv2.WINDOW_AUTOSIZE)
# cv2.imshow("view2",view2)
# view3 = cv2.warpPerspective(view3, H3, (w3, h3))
# cv2.namedWindow('view3',cv2.WINDOW_AUTOSIZE)
# cv2.imshow("view3",view3)
# #6. Checking for keyboard interrupt
# q=-12
# q = cv2.waitKey(0)
# if q > 0:
#     print("dest")
...
out11= cv2.VideoWriter('task5_output11.avi',cv2.VideoWriter_fourcc(*'MJPG'), 10, size1)
out22= cv2.VideoWriter('task5_output21.avi',cv2.VideoWriter_fourcc(*'MJPG'), 10, size2)
out33= cv2.VideoWriter('task5_output31.avi',cv2.VideoWriter_fourcc(*'MJPG'), 10, size3)
...
# # #10. Making sure each fram is valid
    if frame1 is not None and frame2 is not None and frame3 is not None:
        print("ooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooo",frame1.shape)
        f1 = frame1
        f2=frame2
        f3 = frame3
...
#12. Getting neural network predictions
    ...
    view1 = cv2.warpPerspective(f1, H1, (576, 768))
    view2 = cv2.warpPerspective(f2, H2, (576, 768))
    view3 = cv2.warpPerspective(f3, H3, (576, 768))
    temp1=[]
    for i in range(len(boxes1)):
        if i in indexes1:
            ...
            print("updated with perspective  1",ouuu)
            label = str(classes[class_ids1[i]])
            x1=int(ou[0]+0.5)
            y1=int(ou[1]+0.5)
            if (x1>10 and y1<566 and y1 > 10 and x1<750):
                temp1.append([y1,x1])
                obj = [x1,y1]
    ...
    for i in range(len(boxes2)):    
        if i in indexes2:
            ...
            print("updated with perspective 2",ouuu)
            label = str(classes[class_ids2[i]])
            x1=int(ou[0]+0.5)
            y1=int(ou[1]+0.5)
            if (x1>10 and y1<566 and y1 > 10 and x1<750):
            ...
            print("printing frame2")
    points2.append(temp1)
    temp1=[]
    for i in range(len(boxes3)): 
        if i in indexes3:
            ...
            print("updated with perspective  3",ouuu)
            ...
            print("printing frame3")
    points3.append(temp1)
    ...
    view1=heat_map(view1,points1,gauss_kernel)
    view2=heat_map(view2,points2,gauss_kernel)
    view3=heat_map(view3,points3,gauss_kernel)
...
# def compute_midpoints(boxes1,boxes2,boxes3,Hs):
#     projected_mid1 = []
#     projected_mid2 = []
#     projected_mid3 = []
#     for box1 in boxes1:
...
#         new_mid = project_midpoint_into_top_view(midpoint,Hs[0])
#         projected_mid1.append(new_mid)
#     for box2 in boxes2:
#         x = box2[0]
#         y = box2[1]
#         w = box2[2]
...
#         mid_y = (y+h)/2
#         midpoint = [mid_x,mid_y]
#         new_mid = project_midpoint_into_top_view(midpoint,Hs[1])
#         projected_mid2.append(new_mid)
#     for box3 in boxes3:
#         x = box3[0]
#         y = box3[1]
#         w = box3[2]
...
#         mid_y = (y+h)/2
#         midpoint = [mid_x,mid_y]
#         new_mid = project_midpoint_into_top_view(midpoint,Hs[2])
#         projected_mid3.append(new_mid)
#     return projected_mid1,projected_mid2,projected_mid3
